...
def train(data, img_flag):
	...
	log_filename = f"AutoEncoder_{args.type_flag}_training_{timestamp}.log"
	...
	logger.info(f"CrossFuse Training Log - AutoEncoder_{args.type_flag}")
	...
	for e in range(args.epochs):
		...
		logger.info(f"Epoch {e+1} Summary: recon_loss: {loss_p4:.6f} - ssim_loss: {loss_p5:.6f} - total_loss: {loss_all:.6f}")
		logger.info("-" * 60)
		logger.info("")
		...
		model.train()
		model.cuda()
		logger.info(f"Checkpoint, trained model saved at: {save_model_path}")
